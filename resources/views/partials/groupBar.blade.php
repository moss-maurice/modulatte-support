...
<div class="text-center d-flex justify-content-center align-items-baseline">
        @if ($tab->groupBar()->isNotEmpty())
            @foreach ($tab->groupBar() as $index => $button)
                <button type="submit" name="groupAction" value="{{ $button->getProp('name') }}" id="group-bar-button-{{ ($index + 1) }}" class="btn btn-{{ $button->getStyle() }} btn-sm inactive styled rounded px-3 py-2 border-0" href="{{ $button->getTarget() }}"{!! $button->getPropsToLine() !!}>
                    @if ($button->hasIcon())
