...
<td class="tableItem {{ $name }} {{ $class }}" data-field="{{ $name }}" {{ ($colspan > 1 ? " colspan=\"{$colspan}\"" : '') }} {{ ($rowspan > 1 ? " rowspan=\"{$rowspan}\"" : '') }}>
    <div class="row">
        <label class="switch mx-auto col-auto" id="{{$item->id}}">
            <input type="checkbox" name="{{$name}}" value="{{$value}}" @if ($value) checked @endif data-link="/manager/?a=112&id={{request()['id']}}&tab={{$tab->slug()}}&method=changeEnable&itemId={{$item->id}}" />
